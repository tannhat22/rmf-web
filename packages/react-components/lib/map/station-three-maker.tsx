...
interface StationThreeMakerProps {
  position: [number, number, number];
  color: string;
  text?: string;
  circleShape: boolean;
  onStationClick?: (ev: ThreeEvent<MouseEvent>) => void;
}

export const StationThreeMaker = ({
  position,
  color,
  text,
  circleShape,
  onStationClick,
}: StationThreeMakerProps): JSX.Element => {
  const HEIGHT = 8;
  const ELEVATION = 0;
      ...
      {circleShape ? (
        ...
        <group position={position}>
          <mesh
            ...
            onClick={(ev: ThreeEvent<MouseEvent>) => onStationClick && onStationClick(ev)}
          >
            {isHovered && (
              <Html zIndexRange={[1]}>
                <div
